...
describe('dropRight', () => {
  ...
  test.each([
    ...
    (arg1, arg2) => {
      const result = dropRight(arg1, arg2) as Error;
      const errData = result.cause as ErrData;
  ...
  test('should return an empty array if n is greater than arr.length', () => {
    const result = dropRight([1, 2, 3], 7);
  ...
  test('should make a shallow copy of objects inside arr', () => {
    const arg = [{ name: 'Cindy' }, { name: 'Bob' }];
    const result = dropRight(arg, 0) as object[];
  ...
  test.each([
    [{ 0: 'a', 1: 'b', 3: 'c', length: 4 }, 1, ['a', 'b', ,]], // eslint-disable-line no-sparse-arrays
    [[1, , 2, 3, 5], new Number(4), [1]], // eslint-disable-line no-sparse-arrays
    ['string', 2, ['s', 't', 'r', 'i']],
  ])('dropRight(%s, %s) should equal %s', (arg1, arg2, expected) => {
    expect(dropRight(arg1 as ArrayLike<unknown>, arg2 as number)).toStrictEqual(
      expected,
    );
  });
});
